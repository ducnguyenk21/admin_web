...
const WorkoutReviewDialog: React.FC<Props> = ({ open, onClose, workoutId }) => {
  ...
  const reloadReviews = async () => {
    ...
    const data = await fetchReviews(workoutId, userMapObj);
    setReviews(data);
    setLoading(false);
  };
  ...
  const handleSendReply = async (review: Review) => {
    if (!workoutId) return;
    const reviewDoc = doc(db, `Workouts/${workoutId}/Reviews`, review.userId);
...
export default WorkoutReviewDialog;
